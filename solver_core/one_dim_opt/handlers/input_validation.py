...
def check_limits(limits: str, split_by: Optional[str] = None) -> str:
    """
    Эта функция проверяет корректность введенных ограничений для переменных.
    Поддерживает операции с pi и e.

    Parameters:
    ------------
    limits: str
        Строка, содержащая ограничения слева и справа для переменной.
...
def check_float(value: str) -> float:
    ...
    -------
    float
        Значение, переведенное из строки в тип float.
    """
...
def check_int(value: str) -> int:
    ...
    -------
    int
        Значение, переведенное из строки в тип int.
    """
